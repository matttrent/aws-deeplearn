...
# fi
anaconda_short_python_version="anaconda"
...
if [[ ! -e "$HOME/$anaconda_short_python_version" ]]; then
    ...
    bash "${anaconda_python_version}-Linux-x86_64.sh" -b -p $HOME/$anaconda_short_python_version
fi
...
bcolz ipython scipy numpy pandas scikit-learn pillow h5py py-xgboost dask \
matplotlib seaborn jupyter notebook gensim nltk
